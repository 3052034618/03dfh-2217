...
function getProductInfo(code) {
  ...
  return T.find(p => p.code === code) || T[0];
}

function randomInt(m, n) { return Math.floor(Math.random() * (n - m + 1)) + m; }
function randomFloat(m, n, d = 1) { return parseFloat((Math.random() * (n - m) + m).toFixed(d)); }
function randomChoice(a) { return a[Math.floor(Math.random() * a.length)]; }

function calcRiskScore(maxTemp, threshold, tempTrend, pt) {
  let s = 0;
  const d = maxTemp - threshold;
  if (d > 5) s += 50; else if (d > 3) s += 35; else if (d > 0) s += 20; else if (d > -2) s += 5;
  if (tempTrend === 'rising_fast') s += 35; else if (tempTrend === 'rising') s += 20; else if (tempTrend === 'stable') s += 5; else s -= 5;
  if (pt === 'VACCINE') s += 15; else if (pt === 'ICE_CREAM') s += 10; else if (pt === 'FROZEN_SEAFOOD') s += 8;
...
function generateMockVehicles() {
  ...
  const vs = [];
  for (let i = 0; i < 12; i++) {
    const p = randomChoice(PT);
    const tc = randomChoice(['rising_fast', 'rising', 'stable', 'falling']);
    let mt;
    if (tc === 'rising_fast') mt = randomFloat(p.threshold + 1, p.threshold + 8);
    ...
    const etaMin = randomInt(2, 45);
    const now = new Date();
    const eta = new Date(now.getTime() + etaMin * 60000);
    const rs = calcRiskScore(mt, p.threshold, tc, p.code);
    const rl = getRiskLevel(rs);
    const st = etaMin <= 5 ? 'approaching' : etaMin <= 15 ? 'waiting' : 'en_route';
    vs.push({
      id: 'VH' + String(1001 + i),
      plateNumber: PN[i % PN.length],
      driver: randomChoice(DR),
      supplier: randomChoice(SU),
      productType: p.code,
      productName: p.name,
      standardTemp: p.standardTemp,
      thresholdTemp: p.threshold,
      maxTemp: mt,
      tempTrend: tc,
      trendLabel: getTrendLabel(tc),
      riskScore: rs,
      riskLevel: rl,
      eta: eta,
      etaMinutes: etaMin,
      status: st,
      weight: randomFloat(5, 28),
      ...
      assignedDock: rl === 'critical' ? randomInt(1, 4) : null,
...
function generateTemperatureHistory(maxTemp, tempTrend, productType) {
  const p = getProductInfo(productType);
  const h = [];
  const now = new Date();
  const base = p.standardTemp;
  const N = 30;
  for (let i = N - 1; i >= 0; i--) {
    ...
    let temp;
    switch (tempTrend) {
      case 'rising_fast': temp = base + (maxTemp - base) * Math.pow(pr, 0.7) + randomFloat(-0.5, 0.5); break;
...
const mockAPI = {
    ...
    onUpdated: (cb) => {
      setInterval(() => {
        mockState.vehicles = mockState.vehicles.map(v => {
          if (v.etaMinutes > 0) return { ...v, etaMinutes: v.etaMinutes - 1, eta: new Date(Date.now() + (v.etaMinutes - 1) * 60000) };
          return v;
        });
        cb(JSON.parse(JSON.stringify(mockState.vehicles)));
      }, 60000);
    },
    onSelected: (cb) => {
      detailListeners.push(cb);
      if (mockState.detailVehicleId) cb(mockState.detailVehicleId);
      if (typeof window !== 'undefined' && window.mockVehicleId && !mockState.detailVehicleId) {
  ...
  records: {
    ...
    onInit: (cb) => {
      ...
      if (typeof window !== 'undefined' && window.mockVehicleId && !mockState.recordVehicleId) {
        cb(window.mockVehicleId);
      }
    },
    onCreated: () => {}
  ...
  window: {
    openDetail: (id) => {
      mockState.detailVehicleId = id;
      const w = typeof window !== 'undefined' ? window.open('detail.html', '车辆详情', 'width=960,height=720') : null;
      if (w) { w.mockVehicleId = id; setTimeout(() => detailListeners.forEach(cb => cb(id)), 300); }
      detailListeners.forEach(cb => cb(id));
    },
    openRecord: (id) => {
      mockState.recordVehicleId = id;
      const w = typeof window !== 'undefined' ? window.open('record.html', '收货记录', 'width=720,height=800') : null;
...
if (!isRunningInElectron() && typeof window !== 'undefined' && !window.electronAPI) {
